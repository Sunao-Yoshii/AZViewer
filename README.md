...
Stable Diffusion WebUI で生成した画像のメタ情報、プロンプト由来タグ、caption ファイル由来タグ、生成元モデルを扱いやすくすることを主な目的にしています。画像をタイル表示しながら、タグやモデルで絞り込み、必要に応じて Stable Diffusion WebUI 用のワイルドカード `.txt` や、LoRA 作成向けの caption `.txt` も出力できます。
...
- リリースページ: https://github.com/Sunao-Yoshii/AZViewer/releases/tag/1.3.0
...
- 画像と同名の caption `.txt` を読み取り、既存タグへ追加登録
- 選択画像に対するタグの一括追加
- 生成元モデル名の登録、表示、検索
- パス、レーティング、チェック状態、お気に入り、タグ、タグ部分一致、フォルダ、モデルによる検索
- 同じタグ構成を持つ画像の検索
- 表示中画像の一括選択、一括選択解除
...
- 選択画像のタグを画像と同名の caption `.txt` として出力
- タグ、モデルのマスタメンテナンス
...
キャプションタグ読み込みと一括タグ追加では、既存タグは削除されず、読み込んだタグまたは入力したタグが追加されます。重複するタグは二重登録されません。
...
- 表示中をすべて選択: 現在表示されている画像をまとめて選択します。表示中の画像がすべて選択済みの場合は、表示中の選択を解除します。
...
- ファイル操作 > 選択画像を削除: AZViewer 上の登録情報、サムネイルキャッシュ、実際の画像ファイルをまとめて削除します。
...
- タグメンテナンスやモデルメンテナンスで使用中のタグ・モデルを削除すると、対象画像からそのタグ・モデルの関連付けが外れます。実行前に表示される影響件数を確認してください。
- 画像メタ情報はファイル形式や生成環境によって異なります。すべての画像でプロンプトやモデル名を取得できるとは限りません。
- メタデータからのプロンプト読み取りも行いたい場合は、caption ファイルの読み込みより先に「プロンプトの読み取り」を実行してください。caption ファイルを先に読み込むと対象画像にタグが追加されるため、その画像はプロンプト一括読み取りの対象外になります。
- キャプションタグ読み込みは、選択画像と同じフォルダにある同名 `.txt` を読み込み、既存タグへ追加します。caption ファイルが存在しない画像や、空の caption ファイルはスキップされます。
